...
is_hol = setup9[(setup9['plastic'] == 0) | (setup9['plastic'] == 2)]
...
print(vectormaker['assay'].value_counts())
...
class myRNN(nn.Module):
    def __init__ (self, x ,  lags, layers,epochs):
        ...
        #kk
        self.linearlayer = Variable(self.gloroti(48,192).type(self.dtype),requires_grad=True)
        self.finallayer= Variable(self.gloroti(192,3).type(self.dtype),requires_grad=True)
        self.linearbias = Variable(torch.zeros(1,192).type(self.dtype),requires_grad=True)
        self.finalbias = Variable(torch.zeros(1,3).type(self.dtype),requires_grad=True)
...
self.w_ibias],lr = 2.5e-4 )
    ...
    def dataset(self):
        ...
        minibatch = torch.zeros(24,self.lags, self.layers[0])
    ...
    def forwardpass(self, g1):
        ...
        g1 = F.relu(torch.matmul(g1,self.linearlayer)+self.linearbias)
        g1 = torch.matmul(g1,self.finallayer)+self.finalbias
    ...
    def descendagradient(self):
        ...
        for i in range(self.epochs):
            ...
            if (i+1)% 10 == 0:
                ...
                print("low plasticity" , checker[1][:,0].sum())
                print("")
                print("medium plasticity" , checker[1][:,1].sum())
                print("")
                print("high plasticity", checker[1][:,2].sum())
                print("")
                ...
                print(self.accuracycheck(loader[0], checker[1])[2])
