...
class myRNN(nn.Module):
    def __init__ (self, x ,  lags, layers,conv_layers, epochs, minibatch):
        ...
        self.linearlayer = nn.Linear(25,10).cuda()
        self.finallayer = nn.Linear(10,2).cuda()
        self.dropeout = nn.Dropout(p=0.2)
        ...
        self.optimizeruno = torch.optim.Adam(self.params,lr=5e-5)
...
self.w_ibias,
        ...
        removal = torch.zeros(250,self.minibatch,1,self.lags,6).cuda()
        checker = torch.zeros(250,self.minibatch,2).cuda()
        checkerenv = torch.zeros(250,self.minibatch,12).cuda()
    ...
    def forwardpass(self, g1):
        ...
        h = self.dropeout(F.relu(self.linearlayer(h)))
        h = self.dropeout(self.finallayer(h))
    ...
    def descendagradient(self):
        ...
        for i in range(self.epochs):
            ...
            if (i+1)% 100 == 0:
                pick = np.random.randint(0,249,1)
                ...
                print("low plasticity or medium" , acc[:,0].detach().cpu().numpy().sum())
                print("")
                print("high plasticity", acc[:,1].sum())
                print("")
                print(inpt[0].detach().cpu().numpy())
                acc0, acc1, acc2 = self.accuracycheck(checkem,acc)
                print( "Current Accuracy" , acc0)
    ...
    def accuracycheck(self, x, y ):
        ...
        summable = 0
        confused = np.zeros((len(loader[0]),len(loader[0])))
        ...
        george = sum(confused[i][i] for i in range(len(confused[0])))
        
            
        
        summable = george/self.minibatch
...
rnn = myRNN(vectormaker,8  ,[4,80],[16,16,32,32],20000, 48)
...
for i in range(250):
    ...
    c0, c1, c2 = rnn.removal[i], rnn.checker[i], rnn.checkerenv[i]
    sums[i] = rnn.accuracycheck(c0,c1)[0]
    trick += rnn.accuracycheck(c0,c1)[2]

print(trick/250)
print("empirical accuracy", (sum(trick[i][i] for i in range(len(trick[1])))/(250*48)))
