...
key = key.loc[key['assay']  == 26].loc[key['plastic'] == 'L'].loc[key['env'] == 'DMSO']
...
for ai, aa in enumerate(groups):
	...
	for si, ss in enumerate(time_ids):
		...
		# print('88')
		# print(subid_pool.loc[subid_pool['trackID'] == 88])
		...
		np.fill_diagonal(pairwise, 99999.)
		...
		for ri, region in enumerate(regionlist):
			...
			if region.area >= 100*4*4: # and np.amax([(maxr-minr),(maxc-minc)]) <= 60*4:
		...
		while (len(uniq(keep_region[:,8])) != len(keep_region[:,8])): # or np.sum(np.isnan(keep_region[:,8]))>0:
			...
			for di, dd in enumerate(dups):
				print('enter')
				if len(keep_region[keep_region[:,8] == dd]) == 4:
					
					print(keep_region[keep_region[:,8] == dd])
					print(keep_region[((keep_region[:,8] == dd) & (keep_region[:,9] != np.amin(keep_region[keep_region[:,8] == dd, 9]))), -2:])
